...
class Approval_model extends CI_Model
{
    ...
    public function get_total_amount($rfq_ID){
      // $this->db->select('quote_summary.*,approval_sheet_summary.*')
      // ->join('quote_summary','quote_summary.rfq_ID=approval_sheet_summary.rfq_ID')
      // ->join('approval_sheet_detail','approval_sheet_summary.rfq_ID=approval_sheet_detail.rfq_ID')
      // ->join('quote_detail','quote_summary.quote_ID=quote_detail.quote_ID')
      // ->where('quote_detail.req_detail_ID=approval_sheet_detail.req_detail_ID')
      // ->where('quote_summary.rfq_ID',$rfq_ID)
      // ->where('approval_sheet_detail.vendor_ID=quote_summary.vendor_ID');
      // return $this->db->get('approval_sheet_summary')->result();
      $this->db->select('SUM(quote_detail.quantity * quote_negotiation.negotiated_price) AS amount')
      ->join('quote_detail','quote_detail.rfq_ID=approval_sheet_detail.rfq_ID')
      ->join('quote_summary','quote_summary.quote_ID=quote_detail.quote_ID')
      ->join('quote_negotiation','quote_detail.rfq_ID=quote_negotiation.rfq_ID')
      ->where('quote_detail.rfq_ID',$rfq_ID)
       ->where('quote_detail.req_detail_ID=approval_sheet_detail.req_detail_ID')
       ->where('quote_summary.vendor_ID=approval_sheet_detail.vendor_ID');
    ...
    public function negotiate_price($data){
      return $this->db->insert('quote_negotiation',$data);
    }
    public function get_negotiated($rfq_id){
     return  $this->db->where('rfq_ID',$rfq_id)
      ->get('quote_negotiation')->result();
    ...
    public function get_quote_sent($rfq_id){
      $this->db->select('quote_detail.*,quote_summary.*');
      $this->db->join('quote_summary','quote_detail.quote_ID=quote_summary.quote_ID');
     return $this->db->where('quote_detail.rfq_ID',$rfq_id)
      ->get('quote_detail')->result();
    }
    public function save_negotiated_detail($nego){
      return $this->db->insert('quote_negotiation',$nego);
    }
    public function update_procure_status($rfq_ID,$reference){
      $this->db->set('procure_forensic_status','1')
       ->set('budget_reference',$reference)
      ->where('rfq_ID',$rfq_ID)
      ->update('approval_sheet_summary');
    }
    public function update_forensic_status($rfq_ID,$reference){
       $this->db->set('procure_forensic_status','2')
       ->set('budget_reference',$reference)
      ->where('rfq_ID',$rfq_ID)
      ->update('approval_sheet_summary');

    }
    public function update_negotiation($data,$id){
      $this->db->set($data)
      ->where('negotiation_ID',$id)
      ->update('quote_negotiation');
    }
    public function rfq_summary_negotiated($id){
      $this->db->set('negotiated','1')
      ->where('rfq_ID',$id)
      ->update('rfq_summary');
    }
    public function get_rfq($id){
     return $this->db->where('rfq_ID',$id)
      ->get('rfq_summary')->row();
    }
    public function get_vendors_mail($id){
      return $this->db->select('e_vendor.*,approval_sheet_detail.*')
      ->join('e_vendor','e_vendor.supplier_id=approval_sheet_detail.vendor_ID')
      ->where('approval_sheet_detail.rfq_ID',$id)
      ->get('approval_sheet_detail')->result();
    }
}