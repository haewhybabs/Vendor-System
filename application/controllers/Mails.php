...
class Mails extends MY_Controller
{
    ...
    public function index()
    {
      ...
      $general_settings=$this->Settings_model->get_all_gen();
         ...
         if ($this->form_validation->run() == FALSE) {
           ...
           // $this->audit($task);
            ...
            for($i=0; $i<count($this->input->post('id')); $i++){
                $key_id=$this->input->post('id')[$i];

                $update=array(
                    'value'=>$this->input->post('settings')[$i]
                );
                $this->Settings_model->gen_update($key_id,$update);
            }
          //  var_dump($this->input->post('id'));
            ...
            $this->session->set_flashdata('message', 'General Settings Edited Successfully');
