  ...
  $task='Set price on purchase order'; 
    ...
    public function sheet_approve(){
        ...
        if ($this->form_validation->run()){
               ...
               //Save to quote_negotiation
               $get_quote_sent=$this->Approval_model->get_quote_sent($this->input->post('rfq_ID'));
               foreach ($get_quote_sent as $key) {
                   $nego['rfq_ID']=$this->input->post('rfq_ID');
                   $nego['req_detail_ID']=$key->req_detail_ID;
                   $nego['negotiated_price']=$key->unit_price;
                   $nego['vendor_ID']=$key->vendor_ID;

                   $this->Approval_model->save_negotiated_detail($nego);
               }

                $this->session->set_flashdata('message', 'Approval Sheet is successfully Created');
                 redirect(site_url('quotes/set_price/'));
                 //var_dump($get_quote_sent);
        }
        else{
    ...
    public function sheet_neg_approve(){
         $this->form_validation->set_rules('approval[]','Select Button','required');
        if ($this->form_validation->run()){
    ...
    public function sheet_procure($id){
         $allquote=$this->Approval_model->approved_vendors($id);
     //var_dump($allquote);
         $data=array(
          'sheet_summary'=>$allquote,
      ...
      $this->template->load('template', 'quotes/sheet_procure', $data);
    }
    public function procure_negotiate($id){
        $list = $this->Approval_model->get_by_requisition($id);
        $allproduct=$this->Approval_model->get_distinct_detail($id);
        $products=$this->Approval_model->approved_vendors($id);
        $allquote=$this->Approval_model->get_allQuote($id);
        $data=array(
            'allproduct'=>$allproduct,
            'allquote'=>$allquote,
            'title'=>'Approval Sheet',
            'sheet_summary'=>$products
        );
        $this->template->load('template', 'quotes/sheet_negotiate_update', $data);
    }
    public function negotiate_price($id){
        $key=$this->Approval_model->get_rfq($id);
        
        if($key->negotiated=='1'){
            redirect('Quotes/set_price');
        }
        $list = $this->Approval_model->get_by_requisition($id);
        $allproduct=$this->Approval_model->get_distinct_detail($id);
        $allquote=$this->Approval_model->get_allQuote($id);
        ...
        $this->template->load('template', 'quotes/sheet_negotiate', $data);

    }
    public function sheet_negotiate(){
        for($count=0; $count<count($this->input->post('price',TRUE)); $count++ ){
            $data=array(
                'rfq_ID'=>$this->input->post('rfq_ID'),
                'req_detail_ID'=>$this->input->post('req_detail_ID')[$count],
                'negotiated_price'=>$this->input->post('price')[$count],
                'vendor_ID'=>$this->input->post('vendor')[$count]
            );
            $this->Approval_model->negotiate_price($data);
        }

        $id=$this->input->post('rfq_ID');
        //set negotiate to 1
        $this->Approval_model->rfq_summary_negotiated($id);
        $list = $this->Approval_model->get_by_requisition($id);
        $allproduct=$this->Approval_model->get_distinct_detail($id);
        $allquote=$this->Approval_model->get_allQuote($id);
        $negotiate=$this->Approval_model->get_negotiated($this->input->post('rfq_ID'));
        $data=array(
            'allproduct'=>$allproduct,
            'allquote'=>$allquote,
            'negotiate'=>$negotiate,
         ...
         $this->session->set_flashdata('message','Operation is Successful');
        $this->template->load('template', 'quotes/sheet_negotiation', $data);


    }
    public function approvalsheet_negotiate($id){
        $list = $this->Approval_model->get_by_requisition($id);
        $allproduct=$this->Approval_model->get_distinct_detail($id);
        $allquote=$this->Approval_model->get_allQuote($id);
        $negotiate=$this->Approval_model->get_negotiated($id);
        $data=array(
            'allproduct'=>$allproduct,
            'allquote'=>$allquote,
            'negotiate'=>$negotiate,
    ...
    public function approval_negotiation(){
        $rfq_id=$this->input->post('rfq_ID');
        for($count=0; $count<count($this->input->post('price',TRUE)); $count++ ){
            $id=$this->input->post('neg_id')[$count];
            $data=array(
               'negotiated_price'=>$this->input->post('price')[$count]

            );
            $this->Approval_model->update_negotiation($data,$id);
        }
         $this->session->set_flashdata('message','Update is Successful');
        redirect('Quotes/sheet_procure/'.$rfq_id);
    }
