...
class Sap extends MY_Controller {
    ...
    {
        $req_list = $this->Requisition_details_model->getSent_requisition_sap();
          $req_list_attended= $this->Requisition_details_model->getSent_requisition_sap_attended();
              ...
              $this->email->set_newline("\r\n");
               ...
               $get_department = $this->Department_model->get_email($dept->department_ID);
                        $recipientArr = array($host->audit_mail, $host->procure_mail, $host->admin_mail, $host->budget_mail, $get_department->email, $get_department->general_email, $host->dfs_mail);
                         ...
                         $data3 = array(
                                    'info'=>'Approval Sheet for supply of requisition items with refence number: '.$dept->reference.' by department of '.$dept->dept_name.' approved by SAP.',
                              'comment'=>'Final Aproval is urgently required to approve requisition for vendor quotations. Thanks',
                                );
                                $option = array(
                                    'subject'   => 'Approval Sheet approved by Sap',
                                ...
                                $this->audit($task);
                                 ...
                                 $this->Requisition_details_model->sap_update($rfq_ID);
                               
                                $this->Requisition_details_model->req_sap_update($req_id);
              ...
              $this->email->set_newline("\r\n");
               ...
               $get_department = $this->Department_model->get_email($dept->department_ID);
                        $recipientArr = array($host->audit_mail, $host->procure_mail, $host->admin_mail, $host->budget_mail, $get_department->email, $get_department->general_email, $host->dfs_mail);
                         ...
                         $data3 = array(
                                    'info'=>'Approval Sheet for supply of requisition items with refence number: '.$dept->reference.' by department of '.$dept->dept_name.'Disapproved by SAP .',
                              'comment'=>'Thanks',
                                );
                                $option = array(
                                    'subject'   => 'Approval Sheet Disapproved by SAP',
                                     'from'      => 'procure@lfcww.org',
                                    'from_name' =>'LFC eProcurement',
                                    'data'      => $data3,
                                );
                                ...
                                $this->Requisition_details_model->sap_update_reject($rfq_ID);
