...
class Request extends MY_Controller {
    ...
    function __construct() {
           ...
           $this->load->library('template');
            $this->load->model(['Department_model', 'Supplier_model', 'Requisition_details_model', 'Vendor_category_model', 'Activities_model', 'Department_requisition_model', 'Vendor_rfq_model', 'Settings_model', 'Audit_model', 'Login_model','Audit_model','Review_Model','Rfq_Model']);
       ...
       // $req_list = $this->Department_requisition_model->get_all_approve();
        ...
        public function send_quote(){
            ...
            $this->form_validation->set_rules('vendor_ID[]','Vendor Option','required');
                   ...
                   $this->session->set_flashdata('error', 'Rfq not sent , please fill the necessary details'); 
                    redirect(site_url('Request/quote_lists'));
                      ...
                      $this->email->set_newline("\r\n");
                       ...
                       $list = $this->Requisition_details_model->get_req_detailsp($data['req_ID']);


                     //Mail for each of the vendors

                       foreach ($vendor as $key) {
                        $supplier = $this->Supplier_model->get_by_id($key);
                          ...
                          $data3 = array(
                                ...
                                //  $this->email->message('hello,how are you doing');
                                ...
                                $data['req_ID']=$this->input->post('req_ID');
                                $get_req_details=$this->Rfq_Model->get_requisition_details_ID($data['req_ID']);
                              ...
                              $store_to_rfqSummary=$this->Rfq_Model->storeRfq($data);
                               ...
                               foreach ($getrfq as $key){
                                ...
                                $rfq_ID=$key->rfq_ID;
                                //Store to rfq_detail with req_detail_ID
                                 foreach ($get_req_details as $key) {
                                  $data=array(
                                    'req_detail_ID'=>$key->req_detail_ID,
                                    'rfq_ID'=>$rfq_ID


                                  );

                                    $this->Rfq_Model->storeRfq_detail($data);
                                 }
                             //  $data['req_ID']=$this->input->post('req_ID');
                                 ...
                                 $req_detail=$this->input->post('req_ID');
                                $update_process_stage=$this->Rfq_Model->update_process_stage($req_detail);
                        // }else{
        ...
        public function Request_quote(){
          ...
          $data['getVendorsDetails']=$this->Rfq_Model->getVendorsDetails();
          ...
          $this->template->load('template', 'requisition/requestQuote', $data);
           //var_dump($data['getVendorsDetails']);
        }

        public function Resend_quote(){
          $this->load->library('pagination');
            $id=$this->input->post('req_ID');
            $cat_id=$this->input->post('category_ID');
            //$req_id=$this->input->post('req_ID');
          ...
          $data['title']='Request Quote';
          $this->session->set_userdata('quote', current_url());
          $this->template->load('template', 'requisition/resendQuote', $data);
           //var_dump($data['getVendorsDetails']);
        }
