...
class Requisition extends MY_Controller {
    ...
    function __construct() {
        ...
        public function index()
        {
            ...
            $this->load->view('templates/Rhistory');
        }
        public function view_history($id){
            $req=$this->Requisition_details_model->get_his_requisition($id);
            $comments=$this->Review_Model->comments($id); 
            $get_budget=$this->Requisition_details_model->get_budget_reference($id);
            $data=array(
                'requisition'=>$req,
                'title'=>'Requisition History Download',
                'comments'=>$comments,
                'budget'=>$get_budget->budget_reference,

            );
            //$this->load->view('templates/Rhistory',$data);

        $html=$this->load->view('templates/Rhistory',$data,TRUE);
        $pdfFilePath = "Activity History012.pdf";
      ...
      public function lists()
        ...
        */$details=$this->Review_Model->Get_details_requisition($this->department_id);
      ...
      public function Activity_history($id){
        ...
        $data['title']='Activity History';
          $this->template->load('template', 'requisition/history', $data);  
            ...
            // }
               ...
               foreach ($getReqId as $key) {
                ...
                if ($getReqId){
                     ...
                     $config = array();
                    $config['upload_path'] = './assets/req_images';
                    ...
                    for($i=0; $i<count($this->input->post('quantity')); $i++){
                        ...
                        $data_req=array(
                            ...
                            'product_specification'=>$this->input->post('specification')[$i],
                            'quantity'=>$this->input->post('quantity')[$i],
                            'req_ID'=>$id,
                            'req_image'=>$files['userfile']['name'][$i],
                        ...
                        $create=$this->Requisition_model->createRequisition($data_req);
                   ...
                   $config = array(
                     ...
                     $data3 = array(
                       ...
                       // $this->email->to('babalolaisaac@gmail.com');
                        ...
                        $this->email->set_alt_message('View the mail using an html email client');
                        $this->email->send(); 
                       // var_dump($get_department->dept_name);
                ...
                // var_dump($this->email->print_debugger());
                  ...
                  $this->session->set_flashdata('error', 'Requisition is Unsuccessfully created'); 
                    redirect(site_url('requisition/news'));
              }
    ...
    public function hod_approve()
      ...
      /*  $data = array(
        ...
        public function Hod_concur(){
             ...
             $req_id=$this->Requisition_details_model->get_byId($id);
           ...
           $config = array(
            ...
            $recipientArr = array($d->general_email, $d->techead_email, $get_department->email, $get_department->general_email, $host->audit_mail, $host->procure_mail);
                ...
                $idr=$this->input->post('req_ID');
                $agree=$this->Review_Model->Hod_agree($datap);
                $concur=$this->Review_Model->HOD_concor($idr);
         ...
         public function Hod_edit(){
          ...
          public function Hod_reject(){
            ...
            if ($this->form_validation->run()){
                ...
                $id=$this->input->post('req_ID');
                $insert_comments=$this->Review_Model->Hod_reject($data);
                $update_req=$this->Review_Model->HodReject($id);
                 $this->session->set_flashdata('message', 'Operation Successful');
                 redirect(site_url('requisition/hod_approve'));

            }
            else{
                $this->session->set_flashdata('error', 'Pls give a reason for rejection');
                 redirect(site_url('requisition/hod_approve'));
            }
    ...
    public function review_service()
       ...
       // $stage=4;
        ...
        public function Review_view($id){
         ...
         public function Review_final($id){

          $data['get_comments']=$this->Review_Model->comments($id); 
          $data['detail_single']=$this->Review_Model->GetSingleDetails($id); 
           $data['title']='Review View';
          $this->session->set_userdata('Review',current_url());
          $this->template->load('template', 'requisition/Review_final', $data);
     ...
     public function tech_head()
       ...
       // $stage=5;
          ...
          $this->template->load('template', 'requisition/tech_list', $data);
     ...
     public function store()
       ...
       $req_list = $this->Requisition_details_model->get_all_store();
        ...
        public function storeCheck_view($id){
            ...
            $this->template->load('template', 'requisition/store_check', $data);
            //var_dump($data);
        ...
        public function store_function(){
            ...
            $this->form_validation->set_rules('store_status[]', 'CheckBox', 'required');

            if ($this->form_validation->run()){

                  for ($i=0; $i<count($this->input->post('store_status',TRUE)); $i++ ){
                    if(!$this->input->post('store_status')[$i]==false){
                        $update_detail=$this->Review_Model->storeFunction($this->input->post('store_status',TRUE)[$i]);
                 ...
                 //  var_dump($this->input->post());
                   ...
                   $data['req_status_check_ID']=$this->input->post('req_status_check_ID');
                   $data['response']=$this->input->post('response');
                   $data['status_checked_by']=$this->input->post('status_check_by');
                   $data['comments']=$this->input->post('comments');


                   $update_summary=$this->Review_Model->store_summary($data['req_ID']);
                   $insert_stage_activities=$this->Review_Model->insert_stage_activities($data);
   ...
   #########################################################################
      ...
      /*  $msg_list= $this->Requisition_details_model->get_msg();    
        ...
        */
        ...
        public function ChurchAdmin_view($id){
               $data['get_comments']=$this->Review_Model->comments($id);
               $data['detail_single']=$this->Review_Model->GetSingleDetailsp($id); 
                $data['title']='Church Administrator';
        ...
        public function ChurchAdmin_reject(){
            $data=$this->input->post();
            $id=$this->input->post('req_ID');
             $update_req=$this->Review_Model->ChurchReject($id);
             $agree=$this->Review_Model->churchAdmin_agree($data);
    ...
    ################################## ALL REQUISITIONS ###############################
     ...
     public function all()
        {    $details=$this->Review_Model->Get_all_details_requisition();
