            ...
            <div class="col-lg-12 portlets">
              <div class="panel">
                <div class="panel-header">
                  <h3><i class="fa fa-table"></i> <strong>PURCHASE ORDER Final Approval</strong></h3>
                </div>
                    ...
                    <table class="table table-dynamic table-tools" data-table-name="Total users">
                      ...
                      <tbody>
                        ...
                        foreach($req_list as $vendor){
                             $no=1;
                             if($vendor->rp_status == 0){
                             $status='<span class="text-warning">Pending</span>';  
                          }
                            if($vendor->rp_status == 1){
                             $status='<span class="text-success">Approved</span>';  
                          }
                            if($vendor->rp_status == 2){
                             $status='<span class="text-danger">Rejected</span>';  
                          }
                            if($vendor->rp_status == 3){
                             $status='<span class="text-danger">On Query</span>';  
                          }
                          $date = strtotime($vendor->rp_receive);
                            ?>
                        <tr>
                        ...
                        <a href="<?= site_url('rp/vendors/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-eye"></span> Vendors</a>
                       ...
                       <?php if($vendor->rp_status == 0 || $vendor->rp_status == 3){?>
                        <a href="<?= site_url('rp/approve/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-thumbs-up"></span> Approve</a>
                        <a href="<?= site_url('rp/query/'.$vendor->id); ?>" class="btn btn-warning btn-sm" ><span class="fa fa-exclamation"></span> Query</a>
                        <a href="<?= site_url('rp/reject/'.$vendor->id); ?>" class="btn btn-danger btn-sm" ><span class="fa fa-trash"></span> Reject</a>   
                         <?php } ?>  
                          </td>
