            ...
            <div class="col-lg-12 portlets">
              <div class="panel">
                <div class="panel-header">
                  <h3><i class="fa fa-table"></i> <strong>PURCHASE ORDER FORENSIC</strong></h3>
                </div>
                    ...
                    <table class="table table-dynamic table-tools" data-table-name="Total users">
                      ...
                      <tbody>
                        ...
                        foreach($req_list as $vendor){
                             $no=1;
                             if($vendor->forensic_status == 0){
                             $status='<span class="text-warning">Pending</span>';  
                          }
                            if($vendor->forensic_status == 1){
                             $status='<span class="text-success">Approved</span>';  
                          }
                            if($vendor->forensic_status == 2){
                             $status='<span class="text-danger">Rejected</span>';  
                          }
                            if($vendor->forensic_status == 3){
                             $status='<span class="text-danger">On Query</span>';  
                          }
                          $date = strtotime($vendor->f_receive);
                            ?>
                        <tr>
                        ...
                        <a href="<?= site_url('forensic/vendors/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-eye"></span> Vendors</a>
                        <a href="#" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-msg<?= $vendor->id; ?>"><span class="fa fa-comments"></span> Comments</a>
                         <?php if($vendor->forensic_status == 0 || $vendor->forensic_status == 3){?>
                        <a href="<?= site_url('forensic/approve/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-thumbs-up"></span> Approve</a>
                        <a href="<?= site_url('forensic/query/'.$vendor->id); ?>" class="btn btn-warning btn-sm" ><span class="fa fa-exclamation"></span> Query</a>
                        <a href="<?= site_url('forensic/reject/'.$vendor->id); ?>" class="btn btn-danger btn-sm" ><span class="fa fa-trash"></span> Reject</a>   
                         <?php } ?>
                          </td>
