...
class Department extends MY_Controller {
    ...
    function __construct() {
           ...
           $this->load->library('template');
            $this->load->model(['Department_model','Supplier_model', 'Activities_model', 'Audit_model', 'Login_model']);
           ...
           //  $array_specialty = implode(",", $array_specialty);
             ...
             $data = array(
                ...
                //'activities_id' =>$array_specialty,
                 'general_email'=>$this->input->post('dept_email'),
                 'tec_email'=>$this->input->post('t_email'),
                 'hod_email'=>$this->input->post('hod_email'),
                  'techead_email'=>$this->input->post('tec_email'),
                );
              $task='Created a department'; 
                $this->audit($task);
         ...
         public function _rules()
    {
	   ...
	   $this->form_validation->set_rules('department_name', 'Department Name', 'trim|required|is_unique[departments.dept_name]',
        ...
        $this->form_validation->set_rules('dept_email', 'Department Email', 'trim|required');
