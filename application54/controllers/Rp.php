...
class Rp extends MY_Controller {
    ...
    public function approve($id){
        ...
        $recipientArr = array($host->audit_mail, $host->procure_mail, $host->admin_mail, $host->budget_mail, $host->store_mail, $host->sap_mail, $host->forensic_mail, $host->rp_mail);
            
        $data3 = array(
            'info'=>'Purchased Order for supply of requisition items with refence number: '.$dept->reference.' by department of '.$get_department->dept_name.' approved for vendor supply.',
                    );
                    $option = array(
                        'subject'   => 'Purchased order approved',
    ...
    public function reject($id)
        {
            ...
            $b_department = $this->Department_model->get_email($this->department_id);
             $dept=$this->Requisition_details_model->get_by_id($id);
            $get_department = $this->Department_model->get_email($dept->department_id);
             $tec = $this->Requisition_details_model->get_rev($id);
        ...
        $this->email->set_newline("\r\n");
            ...
            'supply_status'=>2,
                  ...
                  $data3 = array(
                        'info'=>'Purchase Order for supply of requisition items with reference number: '.$dept->reference.' by department of '.$get_department->dept_name.' rejected for vendor supply',
                    );
                    $option = array(
                    ...
                    $recipientArr = array($host->audit_mail, $host->procure_mail, $host->admin_mail, $host->sap_mail, $host->forensic_mail, $host->rp_mail);
