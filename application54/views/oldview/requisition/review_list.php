            ...
            <div class="col-lg-12 portlets">
              <div class="panel">
                <div class="panel-header panel-controls">
                  <h3><i class="fa fa-table"></i> <strong>TECHNICAL REVIEW OF REQUISITIONS</strong></h3>
                </div>
                    ...
                    <table class="table table-dynamic table-tools table-bordered table-striped" data-table-name="Total users">
                      ...
                      <tbody>
                        ...
                        foreach($req_list as $vendor){
                             $no=1;
                             if($vendor->technical_status == 0){
                             $status='<span class="text-warning">Pending</span>';  
                          }
                            if($vendor->technical_status == 1){
                             $status='<span class="text-success">Requisition Reviewed</span>';  
                          }
                            if($vendor->technical_status == 2){
                             $status='<span class="text-danger">Rejected</span>';  
                          }?>
                        <tr>
                           <td><?= $vendor->reference; ?></td>
                            <td><?= $status; ?></td>
                            <td><?= $vendor->t_receive; ?></td>
                             <td><?= $vendor->t_treated; ?></td>
                          ...
                          <form enctype="multipart/form-data" method="post" action="<?= site_url('requisition/review') ?>" style="display:inline">
                            <input type="hidden" name="id" value="<?= $vendor->id; ?>">
                              <button type="submit" class="btn btn-success btn-sm"><span class="fa fa-check"></span> Review</button>
                          </form>
                          </td>
