            ...
            <div class="col-lg-12 portlets">
              <div class="panel">
                <div class="panel-header">
                  <h3><i class="fa fa-table"></i> <strong>PROCUREMENT COMMITTEE</strong></h3>
                </div>
                    ...
                    <table class="table table-dynamic table-tools table-bordered table-striped" data-table-name="Total users">
                      ...
                      <tbody>
                        ...
                        foreach($req_list as $vendor){
                             $no=1;
                             if($vendor->dfs_status == 0){
                             $status='<span class="text-warning">Pending</span>';  
                          }
                            if($vendor->dfs_status == 1){
                             $status='<span class="text-success">Approved</span>';  
                          }
                            if($vendor->dfs_status == 2){
                             $status='<span class="text-danger">Rejected</span>';  
                          }
                            if($vendor->dfs_status == 3){
                             $status='<span class="text-danger">On Query</span>';  
                          }
                          $date = strtotime($vendor->d_receive);
                            ?>
                        <tr>
                        ...
                        <a href="<?= site_url('dfs/vendors/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-eye"></span> Vendors</a>
                        <a href="#" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-msg<?= $vendor->id; ?>"><span class="fa fa-comments"></span> Comments</a>
                         <?php if($vendor->dfs_status == 0 || $vendor->dfs_status == 3){?>
                        <a href="<?= site_url('dfs/approve/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-thumbs-up"></span> Approve</a>
                        <a href="<?= site_url('dfs/query/'.$vendor->id); ?>" class="btn btn-warning btn-sm" ><span class="fa fa-exclamation"></span> Query</a>
                        <a href="<?= site_url('dfs/reject/'.$vendor->id); ?>" class="btn btn-danger btn-sm" ><span class="fa fa-trash"></span> Reject</a>
                         <?php } ?>   
                          </td>
