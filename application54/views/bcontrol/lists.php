            ...
            <div class="col-lg-12 portlets">
              <div class="panel">
                <div class="panel-header">
                  <h3><i class="fa fa-table"></i> <strong>PURCHASE ORDER BUDGET CONTROL</strong></h3>
                </div>
                    ...
                    <table class="table table-dynamic table-tools table-bordered table-striped" data-table-name="Total users">
                      ...
                      <tbody>
                        ...
                        foreach($req_list as $vendor){
                             $no=1;
                             if($vendor->budget_status == 0){
                             $status='<span class="text-warning">Pending</span>';  
                          }
                            if($vendor->budget_status == 1){
                             $status='<span class="text-success">Approved</span>';  
                          }
                            if($vendor->budget_status == 2){
                             $status='<span class="text-danger">Rejected</span>';  
                          }
                            if($vendor->budget_status == 3){
                             $status='<span class="text-danger">On Query</span>';  
                          }
                          $date = strtotime($vendor->b_receive);
                            ?>
                        <tr>
                        ...
                        <td style="width:53%">
                        <a href="<?= site_url('budget_control/vendors/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-eye"></span> Vendors</a>
                         ...
                         <?php if($vendor->budget_status == 0 || $vendor->budget_status == 3){?>
                        <a href="<?= site_url('budget_control/approve/'.$vendor->id); ?>" class="btn btn-success btn-sm" ><span class="fa fa-thumbs-up"></span> Approve</a>
                        <a href="<?= site_url('budget_control/query/'.$vendor->id); ?>" class="btn btn-warning btn-sm" ><span class="fa fa-exclamation"></span> Query</a>
                        <a href="<?= site_url('budget_control/reject/'.$vendor->id); ?>" class="btn btn-danger btn-sm" ><span class="fa fa-trash"></span> Reject</a> 
                         <?php } ?>  
                          </td>
                       ...
                       <?php 
                            ...
                            if($msg->to == 2){$receiver='Dept. Head';}
                                 if($msg->to == 1){$receiver='Department';}
                                   ...
                                   if($msg->to == 7){$receiver='Procurement Committee';}
                                   if($msg->to == 9){$receiver='SAP';}
                                   if($msg->to == 10){$receiver='Forensic';}
                                   if($msg->to == 11){$receiver='Final Approval';}
                           ...
                           if($msg->sender == 7){$sender='Procurement Commitee';}
                            if($msg->sender == 8){$sender='Budget Control';}
                           if($msg->sender == 9){$sender='SAP';}
                          if($msg->sender == 10){$sender='Forensic';}
                           if($msg->sender == 11){$sender='Resident Pastor';}
